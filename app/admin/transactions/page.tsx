...
export default function TransactionsPage() {
  ...
  // 3. استدعاء الدالة عند أول تحميل للصفحة
  useEffect(() => {
    loadData();
  }, []);

  // 4. بقية الدوال (مثل toggleItem وغيرها)
  // ...

  // 5. شرط التحميل
  if (loading) return <div className="p-10 text-center font-bold">جاري تحميل البيانات من السيرفر...</div>;

  const toggleItem = async (field: string, currentVal: boolean) => {
    if (!currentTransaction) return;
    const newVal = !currentVal;
    
    // Optimistic UI: تحديث الواجهة فوراً قبل رد السيرفر
  ...
  if (!currentTransaction) return <div className="p-10 text-center font-bold">جاري تحميل البيانات من السيرفر...</div>;

  const checklist = [
    { label: "التحقق من الهوية الشخصية", field: "isIdentityVerified", val: currentTransaction.isIdentityVerified },
    { label: "التحقق من صك الملكية", field: "isTitleDeedValid", val: currentTransaction.isTitleDeedValid },
        ...
        <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div className="flex-1 grid grid-cols-2 gap-y-4">
                    ...
                    <div className="col-span-2">
                      ...
                      <b className="text-2xl font-black text-blue-600">
                        ...
                        <span className="text-sm font-normal text-slate-400 ">ل.س</span></b>
                    </div>
                </div>
              </div>
              ...
              <div className="p-6 border-t bg-slate-50/50 flex justify-between items-center">
                <div className="flex gap-3">
                  <button 
                    ...
                    className={`px-10 py-3.5 rounded-xl font-black text-sm shadow-md transition-all ${(!canApprove || currentTransaction.status === 'COMPLETED') ? 'bg-slate-200 text-slate-400' : 'bg-emerald-600 text-white hover:bg-emerald-700 active:scale-95'}`}
                  >
                ...
                <button className="p-3 border rounded-xl bg-white shadow-sm text-slate-400"><Printer size={22}/></button>
              </div>
            </section>
          </main>
          ...
          <aside className="space-y-6">
            <section className="bg-[#051327] p-6 rounded-2xl text-white shadow-2xl ring-1 ring-white/10">
              <h3 className="font-bold mb-6 flex items-center gap-2 text-sm text-right justify-end"><ClipboardList className="text-blue-400" size={18}/> قائمة تدقيق البيانات</h3>
              <div className="space-y-4 text-right">
                {checklist.map((item, idx) => (
                  <div key={idx} onClick={() => toggleItem(item.field, !!item.val)} className="flex items-center justify-end gap-3 cursor-pointer group">
                    <span className={`text-[13px] ${item.val ? 'text-white font-medium' : 'text-white/40 group-hover:text-white/70'}`}>{item.label}</span>
...
function StatCard({ label, value, icon: Icon, color }: any) {
  ...
  return (
    <div className="bg-white p-5 rounded-2xl border flex justify-between items-center shadow-sm">
      <div className="text-right"><p className="text-[10px] font-bold text-slate-400 uppercase">{label}</p><p className="text-2xl font-black text-slate-900 mt-1">{value.toLocaleString()}</p></div>
...
function PartyInfo({ type, name }: any) {
  ...
  return (
    <div className={`p-4 rounded-2xl border-2 text-right ${isSeller ? 'bg-blue-50/50 border-blue-100' : 'bg-emerald-50/50 border-emerald-100'}`}>
      <p className={`text-[10px] font-black uppercase ${isSeller ? 'text-blue-600' : 'text-emerald-600'}`}>{isSeller ? 'البائع (المالك الحالي)' : 'المشتري (المالك الجديد)'}</p>
      <p className="text-sm font-bold mt-1 text-slate-900">{name || "غير محدد"}</p>
      <p className="text-[11px] text-slate-400 mt-1 flex items-center justify-end gap-1"><MapPin size={10}/> دمشق، سوريا</p>
    </div>
  );
}
