...
// قمنا بتحويل الصفحة إلى Server Component للاتصال المباشر بالباك إند وقاعدة البيانات
export default async function Home() {
  // جلب البيانات الحقيقية مباشرة من الباك إند الفعلي قبل رندرة الصفحة
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-right" dir="rtl">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-10">
        {/* Banner الترحيبي الأصلي الخاص بكِ */}
        <section className="bg-[#00B4D8] text-white p-6 md:p-10 rounded-[32px] flex flex-col md:flex-row items-center justify-between shadow-xl relative overflow-hidden">
          <div className="z-10 text-center md:text-right">
            <h1 className="text-2xl md:text-3xl font-black mb-2">أهلاً بك مجدداً، أحمد</h1>
            <p className="text-xs md:text-sm opacity-90 mb-6">لديك 3 عقارات جديدة تهمك اليوم</p>
        ...
        {/* قسم البحث المتقدم الأصلي الخاص بكِ */}
        <section className="relative rounded-[32px] overflow-hidden min-h-[400px] flex items-center justify-center p-4 shadow-2xl">
          ...
          <div className="relative z-10 w-full max-w-4xl bg-white/95 backdrop-blur-md p-6 md:p-8 rounded-[28px] shadow-2xl border border-white/20">
            <h2 className="text-center font-black text-gray-800 text-lg mb-6">محرك البحث المتقدم</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2 flex flex-col gap-1">
                <label className="text-[10px] font-bold text-gray-400 pr-1">المساحة أو الحي</label>
...
{/* قائمة العقارات المربوطة بالباك إند وقاعدة البيانات */}
       {/* قائمة العقارات المربوطة فعلياً وحقيقياً بالباك إند وقاعدة البيانات */}
        <section className="px-2">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-xl font-black text-gray-800">عقارات مميزة</h2>
            <button className="text-blue-500 text-xs font-bold hover:underline">عرض الكل</button>
          </div>
          ...
          {properties && properties.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {properties.map((property: any) => (
                <PropertyCard
                  key={property.id}
                  tag={property.type || "عقار"}
                  color={property.type === "للبيع" || property.type === "شقة للبيع" ? "bg-[#FFB703]" : "bg-green-500"}
                  price={property.price ? property.price.toLocaleString() : "0"}
                  title={property.title}
                  loc={`${property.city || ""}، ${property.location || ""}`}
                  space={property.area ? String(property.area) : "0"}
                  rooms={property.bedrooms ? String(property.bedrooms) : "0"}
                  baths={property.bathrooms ? String(property.bathrooms) : "0"}
                  // قراءة أول صورة من قاعدة البيانات الحقيقية المتصلة بالأكشن
            ...
            <div className="text-center text-gray-500 py-12 bg-white rounded-2xl shadow-sm border border-gray-100">
              <p className="text-sm font-medium">لا توجد عقارات حقيقية مضافة في قاعدة البيانات حالياً.</p>
            </div>
          )}
        </section>
      </main>
...
// مكونات مساعدة أصلية (تطابق تماماً الأكواد والتصميم الأصلي الخاص بكِ)
function SearchField({ label, val }: any) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-[10px] font-bold text-gray-400 pr-1">{label}</label>
...
function PropertyCard({ tag, color, price, title, loc, space, rooms, baths, img }: any) {
  return (
    <div className="bg-white rounded-24px overflow-hidden shadow-sm border border-gray-100 text-right group hover:shadow-xl transition-all duration-300">
      <div className="relative h-48 overflow-hidden">
        <img src={img} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" alt={title} />
        <span className={`absolute top-3 right-3 ${color} text-white text-[10px] font-bold px-3 py-1.5 rounded-full shadow-md`}>{tag}</span>
        <button className="absolute top-3 left-3 p-2 bg-white/80 backdrop-blur rounded-full text-red-500 hover:bg-white transition shadow-md"><Heart size={16} /></button>
      </div>
      <div className="p-5">
        <div className="flex justify-between items-center mb-3">
          <h3 className="font-bold text-gray-800 text-sm truncate max-w-[60%]">{title}</h3>
          <span className="text-[#FFB703] font-black text-sm">{price} ل.س</span>
        </div>
        <p className="text-gray-400 text-[10px] flex items-center gap-1 justify-end mb-4"><MapPin size={10} className="text-blue-400" /> {loc}</p>
